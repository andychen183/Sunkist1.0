...
const char* LinkManager::_defaultUPDLinkName =      "KDrone";
...
void LinkManager::_updateAutoConnectLinks(void)
{
    ...
    QString currentWifiSSID = _defaultUPDLinkName;
#if defined __android__
    QAndroidJniObject str=QAndroidJniObject::callStaticObjectMethod("org/qgroundcontrol/qgchelper/UsbDeviceJNI", "getCurrentWifiSSID", "()Ljava/lang/String;");
     currentWifiSSID = str.toString();
//    qWarning()<<"currentWifiSSID = "<<currentWifiSSID;
#endif
    // Re-add UDP if we need to
    bool foundUDP = false;
    for (int i=0; i<_links.count(); i++) {
        LinkConfiguration* linkConfig = _links.value<LinkInterface*>(i)->getLinkConfiguration();
        if (linkConfig->type() == LinkConfiguration::TypeUdp && linkConfig->name() == currentWifiSSID) {
            foundUDP = true;
            break;
        }
    }
...
//        strncpy(currentWifiSSID,str.toString()+3,str.toString().length()-2);
//        qWarning()<<"currentWifiSSID = "<<currentWifiSSID;

        UDPConfiguration* udpConfig = new UDPConfiguration(currentWifiSSID);
...
LinkConfiguration* LinkManager::createConfiguration(int type, const QString& name)
{
    ...
    if((LinkConfiguration::LinkType)type == LinkConfiguration::TypeSerial)
        _updateSerialPorts();
    qWarning()<<"#ifndef __ios__------------_updateSerialPorts"<<type;
#endif
