...
const char* QGroundControlQmlGlobal::_rcModeKey = "rcMode";
...
//    QSettings settings;
    _rcMode = settings.value(_rcModeKey, 0). toInt();
...
void QGroundControlQmlGlobal::setRcMode(int rcMode)
{
    if (_rcMode != rcMode) {
        QSettings settings;
        settings.setValue(_rcModeKey, rcMode);
        _rcMode = rcMode;
        emit rcModeChanged(rcMode);
    }
}
